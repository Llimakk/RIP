...
# def Index(request):
#     search_bill = request.GET.get("banknotes", "")
#     bills = Bill.objects.all()  # Получаем все купюры из базы данных
#     if search_bill:  # Фильтруем по имени, если введено
#         bills = bills.filter(name__icontains=search_bill)
    
#     # Для демонстрации, давайте будем брать только последние операции
#     pick_operat = Operat.objects.filter(status=1).last()  # Можно изменить на ваше усмотрение
#     cart_count = len(cart)

#     context = {
#         "bills": bills,
#         "banknotes": search_bill,
#         "bills_count": len(pick_operat.bills.all()) if pick_operat else 0,
#         "pick_operat": pick_operat,
#         "cart_count": cart_count
#     }
...
# def bill(request, bill_id):
#     bill_instance = get_object_or_404(Bill, id=bill_id)
#     context = {
#         "bill": bill_instance,
#     }
#     return render(request, "bill.html", context)
...
# def operat(request, operat_id):
#     operat_instance = get_object_or_404(Operat, id=operat_id)
#     context = {
#         "operat": operat_instance,
#         "cart": cart,
#     }
#     return render(request, "operat.html", context)
...
#     if draft_operat:
#         context["cart_count"] = len(draft_operat.get_bills())
#         context["draft_operat"] = draft_operat

#     return render(request, "bills.html", context)


# def add_bill_to_draft_operat(request, bill_id):
...
#         draft_operat.owner = get_current_user()
...
# def clear_cart(request, operat_id):
#     with connection.cursor() as cursor:
#         cursor.execute("UPDATE expeditions SET status = 5 WHERE id = %s", [operat_id])
...
# def operat(request, operat_id):
#     context = {
#         "operat": Operat.objects.get(id=operat_id),
#     }
